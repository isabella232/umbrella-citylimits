...
abstract class Daq_ProjectAbstract
{
    ...
    /**
     * Whatever the_title filter should be used or not
     *
     * @var boolean 
     */
    protected $_doTitle = true;
    ...
    public function execute($content)
    {
        ...
        foreach($this->_apps() as $app) {
            ...
            try {
                ...
                add_filter('the_content', array($this, "theContent"), 9000);
    ...
    public function rewrite($rules)
    {
        ...
        $rconfig = $rdata["config"];
        $rewrites = $rdata["rewrites"];
        ...
        foreach($rewrites as $k => $r) {
            ...
            foreach($r["links"] as $lk => $lv) {
                if(stripos($lv, "/") === false) {
                    $lstatic[] = $lv;
                }
                
                if(stripos($lv, "/id")) {
                    $ldynamic[] = str_replace("/id", "", $lv);
                }
                
                if(stripos($lv, "/slug")) {
                    $lsluged[] = str_replace("/slug", "", $lv);
                }
            } 
            
            if(!empty($r["prefix"])) {
                $prefix = rtrim($r["prefix"], "/") . "/";
            } else {
                $prefix = "";
            }
            
            if(!empty($r["limit_to"])) {
                $limit = join("|", $r["limit_to"]);
            } else {
                $limit = ".?.+?";
            }
            ...
            if(!empty($lstatic)) {
                $lstatic = join("|", $lstatic);
                $rKey = "($limit)/$prefix($lstatic)/?\$";
                $rVal = "index.php?pagename=\$matches[1]&{$param[0]}=\$matches[2]";

                $newrules[$rKey] = $rVal;
            }
            
            if(!empty($ldynamic)) {
                $ldynamic = join("|", $ldynamic);
                $pmatch[] = $param[0].'=$matches[2]';
                $pmatch[] = $rconfig["vars"]["id"].'=$matches[3]';

                $rKey = "($limit)/$prefix($ldynamic)/([0-9]{1,})?\$";
                $rVal = "index.php?pagename=\$matches[1]&".join("&", $pmatch);

                $newrules[$rKey] = $rVal;
            }
            
            if(!empty($lsluged)) {
                $lsluged = join("|", $lsluged);
                $pmatch = array();
                $pmatch[] = $param[0].'=$matches[2]';
                $pmatch[] = $rconfig["vars"]["slug"].'=$matches[3]';

                $rKey = "($limit)/$prefix($lsluged)/([a-z0-9\-_]+)?\$";
                $rVal = "index.php?pagename=\$matches[1]&".join("&", $pmatch);
        ...
        $newrules['(.?.+?)/(category|type)/([^/]+)/?$'] = 'index.php?pagename=$matches[1]&wpjbtag=$matches[2]&wpjbslug=$matches[3]&paged=1';
        $newrules['(.?.+?)/(category|type)/([^/]+)/page/?([0-9]{1,})/?$'] = 'index.php?pagename=$matches[1]&wpjbtag=$matches[2]&wpjbslug=$matches[3]&paged=$matches[4]';
