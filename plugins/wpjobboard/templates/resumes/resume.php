...
<div class="wpjb wpjr-page-resume">
        ...
        <div class="wpjb-top-header-content">
            <div>
                ...
                <em class="wpjb-top-header-subtitle">
                    <?php echo sprintf(__('Last Updated: %1$s (%2$s ago)', "wpjobboard"), wpjb_date_display(get_option('date_format'), $resume->modified_at), daq_time_ago_in_words($resume->time->modified_at)) ?>
                </em>
            </div>
        </div>
    </div>
    ...
    <div class="wpjb-grid wpjb-grid-closed-top">
        <div class="wpjb-grid-row">
            <div class="wpjb-grid-col wpjb-col-30"><?php _e("Address", "wpjobboard") ?></div>
            <div class="wpjb-grid-col wpjb-col-65 wpjb-glyphs wpjb-icon-location">
                    <?php if(wpjb_conf("show_maps") && $resume->getGeo()->status==2): ?>
                    <a href="<?php esc_attr_e(wpjb_google_map_url($resume)) ?>" class="wpjb-tooltip" title="<?php esc_attr_e("show on map", "wpjobboard") ?>"><?php esc_html_e($resume->locationToString()) ?><span class="wpjb-glyphs wpjb-icon-down-open"></span></a>
                    <?php else: ?>
        ...
        <div class="wpjb-grid-row">
            ...
            <div class="wpjb-grid-col wpjb-col-65 wpjb-glyphs wpjb-icon-tags">
                <?php foreach($resume->getTag()->category as $category): ?>
                    <a href="<?php esc_attr_e(wpjb_link_to("category", $category)) ?>"><?php esc_html_e($category->title) ?></a>
                <?php endforeach; ?>
            </div>
        </div>   
        <?php endif; ?>
