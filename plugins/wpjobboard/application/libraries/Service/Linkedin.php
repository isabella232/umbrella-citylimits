...
class Wpjb_Service_Linkedin {
    ...
    public function requestToken($param)
    {
        ...
        return array(
            "url" => self::URL."/uas/oauth/authorize?oauth_token=".$result["oauth_token"],
    ...
    public function profile($fields = null)
    {
        ...
        $endpoint = self::URL.'/v1/people/~'.$fQuery;
        $consumer = new OAuthConsumer($this->param->api_key, $this->param->secret_key, NULL);
        $signatureMethod = new OAuthSignatureMethod_HMAC_SHA1();
    ...
    public static function dispatch($route = null)
    {
        ...
        if(!is_wpjb() && !wpjb_is_routed_to("index.single") && $post_type != "job") {
            return $route;
        }
        ...
        if($applyFromLinkedin) {
            ...
            if($post_type == "job") {
                ...
                $job = Wpjb_Project::getInstance()->getApplication("frontend")->controller->getObject();
            }

            if(Wpjb_Model_Resume::current()) {
                $user_id = Wpjb_Model_Resume::current()->user_id;
            }
    ...
    public static function footer()
    {
        ...
        jQuery(function($) {
            $(".wpjb-linkedin-request-token").click(function(e) {
                ...
                wpjb_overlay_reposition("#wpjb-linkedin-connect")
