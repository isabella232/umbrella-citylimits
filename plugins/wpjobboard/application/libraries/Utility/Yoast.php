...
class Wpjb_Utility_Yoast {
    ...
    public function wp() 
    {
        ...
        add_filter("wpseo_canonical", array($this, "url"));
    ...
    public function url($url)
    {
        ...
        if(wpjb_is_routed_to("index.single") && isset($ph->job->meta->job_source) && $ph->job->meta->job_source->value()) {
            return $url;
        } elseif(wpjb_is_routed_to("index.single")) {
            return wpjb_link_to("job", $ph->job);
        } elseif(wpjb_is_routed_to("index.view", "resumes")) {
            return wpjr_link_to("resume", $ph->resume);
        } else {
            return false;
        }
    }
